...
public class MainActivity extends AppCompatActivity  implements PopupMenu.OnMenuItemClickListener{
    ...
    private void filter(String newText) {
        ...
        for (Notes single_notes : notes){
            if (single_notes.getUser().toLowerCase().contains(newText.toLowerCase())
            || single_notes.getEmail().toLowerCase().contains(newText.toLowerCase())){
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode==101){
            ...
            if (resultCode==Activity.RESULT_OK){
                Notes new_notes= (Notes) data.getSerializableExtra("note");
                database.dao().update(new_notes.getID(),new_notes.getUser(),new_notes.getEmail());
                notes.clear();
                notes.addAll(database.dao().getAll());
                notesListAdapter.notifyDataSetChanged();
            }
